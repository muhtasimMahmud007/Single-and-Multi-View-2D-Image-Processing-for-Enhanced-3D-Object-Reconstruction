...
def train_net(cfg):
    ...
    full_dataset = train_dataset_loader.get_dataset(utils.data_loaders.DatasetType.TRAIN, cfg.CONST.N_VIEWS_RENDERING, train_transforms)
    subset_size = int(1 * len(full_dataset))
    ...
    full_dataset = val_dataset_loader.get_dataset(utils.data_loaders.DatasetType.VAL, cfg.CONST.N_VIEWS_RENDERING, val_transforms)
    ...
    if torch.cuda.is_available():
        print("[INFO] %s Set CUDA_VISIBLE_DEVICES to %s" % (dt.now(), cfg.CONST.DEVICE))
    ...
    output_dir = os.path.join(cfg.DIR.OUT_PATH, '%s', formatted_timestamp)
    output_dir = os.path.normpath(output_dir)

    log_dir = output_dir % 'logs'
    ckpt_dir = output_dir % 'checkpoints'

    # Ensure the creation of the directory
